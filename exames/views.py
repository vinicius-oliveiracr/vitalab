...
def acesso_medico(request, token):
    ...
    if acesso_medico.status == "Expirado":
        messages.add_message(request, constants.ERROR, "Chave de acesso expirada, solicite nova chave de acesso.")
        return redirect('/usuarios/login')
    
    pedidos = PedidosExames.objects.filter(usuario=acesso_medico.usuario).filter(data__gte = acesso_medico.data_exames_iniciais).filter(data__lte = acesso_medico.data_exames_finais).filter(usuario=acesso_medico.usuario)
